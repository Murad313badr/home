...
def filter_by_state(operations: List[Dict], state: str = "EXECUTED") -> Union[List, str]:
    ...
    operation_list = [operation for operation in operations if operation.get("state", "UNKNOWN") == state]
...
def sort_by_date(operations: List[Dict], ascending: bool = True) -> Union[List, str]:
    ...
    if len(operations) == 0:
        ...
        sort_by_date_list = sorted(operations, key=lambda operation: operation["date"], reverse=ascending)
        return sort_by_date_list


operation_list = filter_by_state(operations, state="EXECUTED")

print(operation_list)

sort_by_date_list = sort_by_date(operations, ascending=True)
